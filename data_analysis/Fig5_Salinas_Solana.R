...
if(postscript_file){postscript("Fig5.eps", 
                               horizontal = FALSE, onefile = FALSE, 
                               paper = "special", height = 5.51, 
                               width =5.51)}else{
                    png("Fig5.png", width = 9.5, height = 20, 
                                     units = 'cm', res = 600)}
...
for (f in dates){
 ...
 rhizome_length[i, 3] <- length(data_observed_2000[which(data_observed_2000[, 1]==f & 
              data_observed_2000[, 2]==r), 3])
 i <- i+1
 rhizome_length[i, 1] <- f
 rhizome_length[i, 2] <- "simulated"
 rhizome_length[i, 3] <- length(data_simulated_2000[which(data_simulated_2000[, 1]==f &
...
plot(1:tiempos, rep(1, tiempos), 
  type='n', xaxt = "n", ylim=c(5, 35), 
  ylab="Number of phytomers per rhizome", xlab="", main="Year 2000")
grid()
for (t in 1:tiempos){
...
data_simulated_2018 <- read.csv("outs2018/ecol_mat_nod_founding_rhizomes_2018_environment_2018_cannal_200m_broad_4m_prof_25.csv", header=T, stringsAsFactors = F)
...
for (d in dates_2018){
 ...
 rhizome_length_2018[i, 3] <- length(data_observed_2018[which(data_observed_2018[, 1]==d & 
                data_observed_2018[, 2]==r), 3])
 i <- i+1
 rhizome_length_2018[i, 1] <- d
 rhizome_length_2018[i, 2] <- "simulated"
 rhizome_length_2018[i, 3] <- length(data_simulated_2018[which(data_simulated_2018[, 1]==d & 
...
plot(1:times2018, rep(1, times2018), 
  type='n', xaxt = "n", ylim=c(5, 35), 
  ylab="Number of phytomers per rhizome", xlab="", main="Year 2018" )
grid()
